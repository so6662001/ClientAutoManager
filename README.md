...
主界面工具栏「纸型管理」入口，用于管理本机打印服务器的纸型（Windows 表单），支持三类纸型：

- **一式二等份 / 一式三等份**：输入整张纸 宽×高，按 **整张尺寸** 注册为 Windows 表单，并记录等份数（每份高 = 高 / 等份数）。
- **自定义纸型**：自定义名称、宽×高与可打印边距。

特性：
- **判重**：新增/编辑时先按名称、再按"类别+宽+高+等份数"判重；命中即拦截并提示，编辑时排除自身。
- **已存在列表**：来自打印服务器 `EnumForms`，区分自建与系统内置（内置不可改/删）。
- **权限**：增 / 改 / 删 为机器级写操作，通过一次 **UAC 提权子进程**完成（`--paper-op` 模式）；查询免提权。
- 实现：`Core/PaperForms`（winspool P/Invoke + 判重/校验/等份元数据持久化），`App` 纸型管理窗口。
